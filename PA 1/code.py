...
for index in range(rangeValue):
    x_train[index],x_test[index],y_train[index],y_test[index]=train_test_split(Xs[index], ys[index], 
           test_size=1/3 , random_state=int('050'))

for index in range(rangeValue):
    count=1
    for depth in range(untilDepth):
        tree = DecisionTreeClassifier(criterion='gini',max_depth=count,
                                      random_state=int('042'))        
        tree.fit(x_train[index],y_train[index])
        train_ac[index,depth]=tree.score(x_train[index],y_train[index])
        test_ac[index,depth]=tree.score(x_test[index],y_test[index])
        count+=1
